...
export function isValidPulse(pulseString) {
  ...
  if (isNaN(pulseString)) return false;

  if (pulseString < 30 || pulseString > 250) return false;
...
export function isValidPressure(pressureString) {
  ...
  if (topNumber < 70 || topNumber > 190) return false;
...
export function isValidHemoglobin(hemoglobinString) {
  // First check if the input is a number
  if (isNaN(hemoglobinString)) return false;

  if (hemoglobinString < 12 || hemoglobinString > 17.5) return false;
